...
class WelcomeController extends Controller {
	...
	public function index()
	{
		$product = DB::table('products')
                        ->join('sales', 'products.sale_id', '=', 'sales.id')
		...
		$product_lastest = DB::table('products')
                            ->join('sales', 'products.sale_id', '=', 'sales.id')
                            ->select('products.id as id','products.name as name','products.image as image','products.price as price','products.alias as alias','products.view as view','sales.discount as discount')
                            ->orderBy('view','DESC')->skip(0)->take(4)->get();
	...
	public function loaisanpham($id)
	{
    	$product_cate = DB::table('products')
            ->join('sales', 'products.sale_id', '=', 'sales.id')
            ->select('products.id as id','products.name as name','products.image as image','products.price as price','products.alias as alias','products.view as view','products.cate_id as cate_id','sales.discount as discount')
	...
	public function chitietsanpham($id)
	{
		...
		$product_detail = DB::table('products')
                            ->join('sales', 'sales.id', '=', 'products.sale_id')
                            ->join('colors', 'colors.id', '=', 'products.color_id')
                            ->join('sizes', 'sizes.id', '=', 'products.size_id')
                            ->join('brands', 'brands.id', '=', 'products.brand_id')
                            ->join('madeins', 'madeins.id', '=', 'products.made_id')
                            ->select('products.id as id',
                                'products.cate_id as cate_id',
		...
		$product_cate = DB::table('products')
                        ...
                        ->where('cate_id',$product_detail->cate_id)->where('products.id','<>',$id)
                        ->take(4)
                        ->get();
